...
This package also provides a transport-agnostic MCP tools core for exposing Emacs capabilities to AI backends and adapter code. The current implementation focuses on the reusable core pieces:

- tool registration
- MCP method dispatch for =initialize=, =tools/list=, and =tools/call=
...
You can enable the built-in tool set in Elisp:
...
At this stage, the MCP core is most useful for backend adapters, Lisp-side integrations, and testing. It does not yet start a standalone HTTP listener by itself.

***** Register a Custom Tool

Use =ai-code-mcp-make-tool= to expose a custom Emacs function as an MCP tool:
...
***** Dispatch MCP Methods Directly

You can call the MCP core directly from Elisp:
...
If you need project-specific execution, register a session first and dynamically bind the current session while dispatching:
